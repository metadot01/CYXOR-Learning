    ...
    <section className="relative min-h-[90vh] flex items-center pt-24 overflow-hidden gradient-hero">
      ...
      <div className="section-container relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          {/* Badge */}
          <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-primary/10 border border-primary/20 text-primary font-semibold text-sm mb-8 animate-fade-up shadow-sm">
          ...
          <div className="pt-16 animate-fade-up" style={{ animationDelay: '0.3s' }}>
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-4">
              {["BFSI", "Healthcare", "Government", "Critical Infrastructure", "Technology"].map((item) => (
                <div key={item} className="flex items-center gap-2.5 text-muted-foreground group">
                  ...
                  <span className="text-sm font-semibold group-hover:text-foreground transition-colors">{item}</span>
                </div>
              ))}
            </div>
          </div>
